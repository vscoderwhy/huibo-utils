import timeUtils from "./tools/timeBox";
import storeUtils from "./tools/storeBox";
import eventUtils from "./tools/eventBox";
import numberUtils from "./tools/numberBox";
import stringUtils from "./tools/stringBox";
import matchUtils from "./tools/matchBox";
import ajaxUtils from "./tools/ajaxBox";
import dataUtils from "./tools/dataBox";
import browserUtils from "./tools/browserBox";
import huiboUtils from "./tools/huibo";

export const huiboBox = huiboUtils;
export const timeBox = timeUtils;
export const storeBox = storeUtils;
export const eventBox = eventUtils;
export const numberBox = numberUtils;
export const stringBox = stringUtils;
export const matchBox = matchUtils;
export const ajaxBox = ajaxUtils;
export const dataBox = dataUtils;
export const browserBox = browserUtils;
 ...
 * @returns 生成的UUID字符串
 */
export const createUUID = (len, radix = 0) => {
 ...
 * @returns 转换后的对象
 */
export const anyToString = (o, method) => {
  method && (method = method.toLowerCase());
  Object.keys(o).forEach((k) => {
    if (isType().isObject(o[k]) || isType().isArray(o[k])) {
      o[k] = "";
    }
...
export const isType = () => {
  ...
  // 判断值是否空值
  const isKeyError = (val) => {
    return [null, undefined, "undefined", "null", ""].includes(val);
  };

  const isObjectError = (row) => {
    return isNullOrUnDef(row) ? false : Reflect.ownKeys(row).length > 0;
  };
 ...
 */
export const initLazyIntersectionObserver = (fn) => {
 ...
 */
export const getNameAndType = (val, split = ".") => {
  ...
  if (decimalIndex !== -1) {
    ...
    if (dotNum.length >= 2) {
      if (Number(dotNum[1]) >= 5) {
        return jsCount(numStr.slice(0, decimalIndex + 2), 0.1, "+");
      } else {
...
export const randomWord = () => {
  let code = "";
  for (let i = 0; i < 6; i++) {
    const type = getRandomRound(1, 3);
    switch (type) {
      case 1:
        code += String.fromCharCode(getRandomRound(48, 57)); // 数字
        break;
      case 2:
        code += String.fromCharCode(getRandomRound(65, 90)); // 大写字母
        break;
      case 3:
        code += String.fromCharCode(getRandomRound(97, 122)); // 小写字母
        break;
    }
  }
  return code;
 ...
 * @returns 返回指定范围内的随机整数
 */
export const getRandomRound = (min, max) => {
 ...
 * @returns 转换后的毫秒数
 */
export const getDayValueOfTime = (day = 1) => {
 ...
 */
export const updatePoint = (data, factor) => {
  const p0 = data[0];
  const p1 = data[1];
  const p2 = data[2];
  const p3 = data[3];
 ...
 */
export const transformDecimal = (number, i) => {
  let decimalNum = null;
  // 先转换为数值型
 ...
 * @returns 替换后的路径字符串
 */
export const replacePathParams = (path, name, val) => {
  if (getParams2Object(path)[name]) {
    path = resertUrlParams(path, name, val);
  } else {
    if (path.includes("?")) {
      path = `${path}&${`${name}`}=${val}`;
    } else {
 ...
 */
export const getParams2Object = (url = window.location.href) => {
  if (url.includes("?")) {
 ...
 * @returns 返回重置后的URL地址
 */
export const resertUrlParams = (url, params, value) => {
  let cacheUrl = url.split("?")[0];
  const obj = getParams2Object(url);
 ...
 * @returns 拼接好的带参数的URL
 */
export const splicePageUrlWithParams = (obj) => {
 ...
 */
export const anySetInterval = (fn, t) => {
  let timer;
  function interval() {
    fn();
