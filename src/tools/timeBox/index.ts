...
const timeBox = {
  ...
  getDistanceNow: function (time, unitObj) {
    ...
    if (datetimeCompareStamp - nowStamp > 0) {
      // 未来
      ...
      const distanceDate = Math.ceil(
        (datetimeCompareStamp - nowStamp) / (1000 * 60 * 60 * 24)
      );

      const distanceHour = Math.ceil(
        (datetimeCompareStamp - nowStamp) / (1000 * 60 * 60)
      );

      const distanceMinutes = Math.ceil(
        (datetimeCompareStamp - nowStamp) / (1000 * 60)
      );

      const distanceSeconds = Math.ceil(
        (datetimeCompareStamp - nowStamp) / 1000
      );
      ...
      let fullDistanceMintus =
        Number(getMinutes(time)) - Number(getMinutes(null));
      if (fullDistanceMintus < 0) {
        fullDistanceHour--;
        fullDistanceMintus += 60;
      }
      let fullDistanceSeconds =
        Number(getSeconds(time)) - Number(getSeconds(null));
      ...
      return {
        year:
          distanceYear + distanceUnitObj.yearUnit + distanceUnitObj.afterUnit,
        month:
          distanceMonth + distanceUnitObj.monthUnit + distanceUnitObj.afterUnit,
        date:
          distanceDate + distanceUnitObj.dateUnit + distanceUnitObj.afterUnit,
        hour:
          distanceHour + distanceUnitObj.hourUnit + distanceUnitObj.afterUnit,
        minutes:
          distanceMinutes +
          distanceUnitObj.minutesUnit +
          distanceUnitObj.afterUnit,
        seconds:
          distanceSeconds +
          distanceUnitObj.secondUnit +
          distanceUnitObj.afterUnit,
    ...
    const distanceDate = Math.ceil(
      (nowStamp - datetimeCompareStamp) / (1000 * 60 * 60 * 24)
    );

    const distanceHour = Math.ceil(
      (nowStamp - datetimeCompareStamp) / (1000 * 60 * 60)
    );

    const distanceMinutes = Math.ceil(
      (nowStamp - datetimeCompareStamp) / (1000 * 60)
    );
    ...
    let fullDistanceMintus =
      Number(getMinutes(null)) - Number(getMinutes(time));
    if (fullDistanceMintus < 0) {
      fullDistanceHour--;
      fullDistanceMintus += 60;
    }
    let fullDistanceSeconds =
      Number(getSeconds(null)) - Number(getSeconds(time));
    if (fullDistanceSeconds < 0) {
      fullDistanceMintus--;
      fullDistanceSeconds += 60;
    }
    ...
    return {
      year:
        distanceYear + distanceUnitObj.yearUnit + distanceUnitObj.beforeUnit,
      month:
        distanceMonth + distanceUnitObj.monthUnit + distanceUnitObj.beforeUnit,
      date:
        distanceDate + distanceUnitObj.dateUnit + distanceUnitObj.beforeUnit,
      hour:
        distanceHour + distanceUnitObj.hourUnit + distanceUnitObj.beforeUnit,
      minutes:
        distanceMinutes +
        distanceUnitObj.minutesUnit +
        distanceUnitObj.beforeUnit,
      seconds:
        distanceSeconds +
        distanceUnitObj.secondUnit +
        distanceUnitObj.beforeUnit,
